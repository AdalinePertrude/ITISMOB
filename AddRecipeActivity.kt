...
class AddRecipeActivity : ComponentActivity() {
    ...
    private fun publishRecipe() {
        ...
        DatabaseHelper.addRecipe(recipe) { success ->
            if (success) {
                // If successful in general recipes, also add to user's published recipes
                DatabaseHelper.addRecipeToUserPublished(userName, recipe) { userSuccess ->
                    if (userSuccess) {
                        Toast.makeText(this, "Recipe published!", Toast.LENGTH_SHORT).show()
                        finish()
                    } else {
                        Toast.makeText(this, "Recipe published to general collection but failed to save to your recipes", Toast.LENGTH_SHORT).show()
                        finish()
                    }
                }
            } else {
