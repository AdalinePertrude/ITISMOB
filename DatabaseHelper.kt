...
class DatabaseHelper {
    companion object {
        ...
        fun fetchRecipesByAuthor(userId: String, callback: (List<RecipeModel>) -> Unit) {
            val db = Firebase.firestore
                ...
                .addOnSuccessListener { documents ->
                    ...
                    for (document in documents) {
                        try {
                            ...
                            Log.e("DatabaseHelper", "Error parsing recipe document ${document.id}", e)
                        }
                    }
                    Log.d("DatabaseHelper", "Successfully fetched ${recipes.size} recipes for user: $userId")
                    callback(recipes)
                }
                .addOnFailureListener { exception ->
                    Log.e("DatabaseHelper", "Error fetching recipes by user", exception)
                    callback(emptyList())
                }
        }
        ...
        fun addRecipe(recipe: RecipeModel, onComplete: (Boolean) -> Unit) {
            ...
            db.collection("recipes")
                ...
                .set(recipeData)
                .addOnSuccessListener {
                    Log.d("DB", "Recipe successfully added! Author: ${recipe.author}")
                    onComplete(true)
                }
                .addOnFailureListener { e ->
        ...
        fun addRecipeToUserPublished(userId: String, recipe: RecipeModel, onComplete: (Boolean) -> Unit) {
            val db = Firebase.firestore
            ...
            db.collection("users")
                ...
                .set(recipeData)
                .addOnSuccessListener {
                    Log.d("DB", "Recipe successfully added to user's Published Recipes! Author: ${recipe.author}, User: $userId")
                    onComplete(true)
                }
                .addOnFailureListener { e ->
                    Log.e("DB", "Error adding recipe to user's Published Recipes", e)
                    onComplete(false)
                }
        }
